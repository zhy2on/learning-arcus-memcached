...
워커 스레드는 로그 버퍼에 기록한 다음, 디스크에 반영된 것을 확인하고 나서 클라이언트에게 완료 응답을 보낸다.

```
워커 스레드: 로그 버퍼에 write → flush 완료 신호 등록 → 다른 요청 처리 (논블로킹)
로그 플러시 스레드: 디스크 flush 완료 → 대기 중인 워커 스레드들에 신호
워커 스레드: 신호 받으면 → 원래 클라이언트에 완료 응답
```

"디스크에 반영된 것을 확인한다"는 표현이 워커 스레드가 그 자리에서 멈춰서 기다린다는 뜻처럼 들리는데, 실제로는 그렇지 않다. 워커 스레드는 flush 완료 알림을 받을 "대기자"로 등록만 해두고, 그 사이에 다른 클라이언트 요청을 처리한다. **동기지만 논블로킹**인 거다.

이때 **group commit** 패턴을 쓴다. 여러 워커 스레드가 각자 flush 대기를 걸어두면, 로그 플러시 스레드가 한 번의 fsync로 전부 한꺼번에 처리한 다음 일괄로 신호를 보낸다. 그래서 동기 모드인데도 성능이 상당히 나온다.
